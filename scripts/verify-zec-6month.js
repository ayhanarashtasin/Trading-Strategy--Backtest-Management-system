/**
 * Verify the LAST-6-MONTHS import (2026-03-15 -> 2026-09-15).
 *
 *   node scripts/verify-zec-6month.js
 */
require("./load-env");
const fs = require("fs");
...
const DIR = "E:/EC/Binance-AggTrades/Test-08-17-Sept/opt/lab_payload_6m";
const CSV = "E:/EC/Binance-AggTrades/Test-08-17-Sept/all_optimized_results_6month.csv";
...
(async () => {
  ...
  const { rows: f } = await Q(
    `SELECT count(*)::int total,
            ...
            count(*) FILTER (WHERE start_date=DATE '2026-03-15' AND end_date=DATE '2026-09-15')::int win,
            count(*) FILTER (WHERE duration_days=184)::int dur,
  ...
  check(f[0].win === t, `window 2026-03-15..2026-09-15 on all ${t}`);
  check(f[0].dur === t, `generated duration_days = 184 on all ${t}`);
  ...
  check(mw[0].lo === 202603 && mw[0].hi === 202609 && mw[0].empty === 0,
    `months confined to 2026-03..2026-09 (lo=${mw[0].lo} hi=${mw[0].hi}, ${mw[0].empty} empty)`);
  ...
  for (let i = 1; i <= 8 && i < csv.length; i++) {
    ...
    const { rows } = await c.query(
      `SELECT total_trades, profit_factor::float pf, max_drawdown_percent::float dd,
        ...
        WHERE backtest_name LIKE 'ZEC 5m AggTrades Last-6M%'
          AND details LIKE $1 LIMIT 1`, [`%Search id ${rid},%`]);
    if (!rows.length) { cdiff++; console.log(`        csv row ${rid} not found in db`); continue; }
    const d = rows[0];
    ccmp++;
    if (d.total_trades !== +fcols[ix("m6_trades")]
      || Math.abs(d.pf - +fcols[ix("m6_profit_factor")]) > 1e-3
      || Math.abs(d.dd - +fcols[ix("m6_max_dd_pct")]) > 1e-2) {
      cdiff++;
      console.log(`        ${rid}: db ${d.total_trades}/${d.pf}/${d.dd} vs csv ${fcols[ix("m6_trades")]}/${fcols[ix("m6_profit_factor")]}/${fcols[ix("m6_max_dd_pct")]}`);
    }
  }
  check(cdiff === 0, `${ccmp} top CSV rows located in the db with matching trades/PF/DD`);

  console.log("\n" + "=".repeat(76));
  console.log("6. FINAL SELECTED over the last 6 months");
  console.log("=".repeat(76));
  const { rows: fin } = await c.query(
    `SELECT s.name, b.test_type, b.start_date, b.total_trades, b.profit_factor,
      ...
      WHERE s.name LIKE '%FINAL SELECTED%' AND b.test_type='Robustness' ORDER BY s.name`);
  ...
  check(fin.length === 2 && fin.every((r) => r.months === 7),
    `both FINAL SELECTED strategies have a last-6M row with 7 monthly buckets`);
